...
// ── Crossref metadata proxy ────────────────────────────────────────────────

const normalizeCrossrefWork = (work) => ({
  title: work.title?.[0] || "",
...
const CROSSREF_HEADERS = {
  "User-Agent": "Flux-Research-Platform/1.0 (mailto:flux@research.app)",
  ...
  router.get(
    ...
    async (req, res) => {
      try {
        ...
        const url = `https://api.crossref.org/works?query=${encodeURIComponent(query)}&rows=${rows}&select=DOI,title,author,container-title,publisher,ISSN,ISBN,volume,issue,page,published,issued,created,type,abstract,URL,score`;
        ...
        const works = (data.message?.items || []).map(normalizeCrossrefWork);
        return res.json({ works, totalResults: data.message?.["total-results"] || 0 });
      } catch (error) {
        return handleServerError(res, error, "Failed to search Crossref");
      }
    },
  );
  ...
  router.get(
    ...
    async (req, res) => {
      try {
        ...
        const response = await fetch(url, { headers: CROSSREF_HEADERS });
        if (!response.ok) {
          if (response.status === 404) return res.status(404).json({ error: "DOI not found" });
          throw new Error(`Crossref API error: ${response.status}`);
        }

        const data = await response.json();
        const work = normalizeCrossrefWork(data.message || {});
        return res.json({ work });
      } catch (error) {
        return handleServerError(res, error, "Failed to lookup DOI");
      }
    },
  );
  ...
  router.put(
    ...
    async (req, res) => {
      try {
        ...
        return handleServerError(res, error, "Failed to save metadata");
      }
    },
  );
};
