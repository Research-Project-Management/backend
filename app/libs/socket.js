...
export function initSocket(server) {
  ...
  _io.on("connection", (socket) => {
    ...
    socket.on("join:page", (pageId) => pageId && socket.join(`page:${pageId}`));
    socket.on("join:project", (projectId) => projectId && socket.join(`project:${projectId}`));
    socket.on("join:workspace", (workspaceId) => workspaceId && socket.join(`workspace:${workspaceId}`));
    socket.on("leave:page", (pageId) => pageId && socket.leave(`page:${pageId}`));
    socket.on("leave:project", (projectId) => projectId && socket.leave(`project:${projectId}`));
    socket.on("leave:workspace", (workspaceId) => workspaceId && socket.leave(`workspace:${workspaceId}`));
    ...
    socket.on("presence:join", ({ roomId, user } = {}) => {
      ...
      _io.to(roomId).emit("presence:update", { roomId, users: getRoomPresence(roomId) });
    });
