...
export class VersionService {
  ...
  async getVersions(pageId: string) {
    const versions = await this.versionRepo.findPageVersions(pageId);
    return { versions };
  }

  async createVersion(pageId: string, userId: string, dto: CreateVersionDto) {
    const page = await this.versionRepo.findPageById(pageId);
  ...
  async restoreVersion(pageId: string, versionId: string) {
    ...
    const page = await this.versionRepo.updatePage(pageId, {
  ...
  async deleteVersion(versionId: string) {
    await this.versionRepo.deleteVersion(versionId);
    return { message: 'Version deleted successfully' };
  }

  async getHistory(pageId: string) {
    const history = await this.versionRepo.findPageVersions(pageId);
    return { history };
  }
}
