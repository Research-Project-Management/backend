...
export class DraftController {
  ...
  @Get()
  @ApiOperation({
    summary: 'Get all draft work items for the current user (My Drafts)',
  })
  async getMyDrafts(
  ...
  @Put(':id')
  @ApiOperation({
    summary: 'Update draft content (Auto-save debounce endpoint)',
  })
  @ApiParam({ name: 'id', description: 'Draft UUID' })
  async updateDraft(
    @Param('id') id: string,
  ...
  @ApiOperation({
    summary:
      'Publish draft into an active work item (cấp mã FLUX-123 & xóa draft)',
  })
  @ApiParam({ name: 'id', description: 'Draft UUID' })
  async publishDraft(
    @Param('id') id: string,
