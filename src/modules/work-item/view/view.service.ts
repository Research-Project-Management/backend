...
export class ViewService {
  ...
  async getViews(
    ...
    return this.viewRepository.findProjectViews(
      projectId,
      userId,
      queryViewDto,
    );
  ...
  async createView(
    ...
    const displayFilters = {
      ...(createViewDto.displayFilters ?? {}),
      ...(createViewDto.layout ? { layout: createViewDto.layout } : {}),
    };
  ...
  async updateView(
    ...
    // Lock check: if view is locked, only creator or project owner can update
    if (
      existing.isLocked &&
      ...
      throw new ForbiddenException(
        'This saved view is locked and can only be modified by its creator or a project owner',
      );
    }

    // Private view check: only creator or project owner can modify
    if (
    ...
    const mergedDisplayFilters =
      updateViewDto.displayFilters !== undefined || updateViewDto.layout !== undefined
        ? {
            ...
            ...(updateViewDto.layout ? { layout: updateViewDto.layout } : {}),
          }
        : undefined;
    ...
    const updated = await this.viewRepository.update(
      viewId,
      {
        ...
        ...(updateViewDto.description !== undefined && {
          description: updateViewDto.description,
        }),
        ...(query !== undefined && { query }),
        ...(filters !== undefined && { filters }),
        ...(mergedDisplayFilters !== undefined && {
          displayFilters: mergedDisplayFilters,
        }),
        ...
        ...(updateViewDto.access !== undefined && {
          access: updateViewDto.access,
        }),
        ...(updateViewDto.sortOrder !== undefined && {
          sortOrder: updateViewDto.sortOrder,
        }),
        ...(updateViewDto.logoProps !== undefined && {
          logoProps: updateViewDto.logoProps,
        }),
        ...(updateViewDto.isLocked !== undefined && {
          isLocked: updateViewDto.isLocked,
        }),
        ...(updateViewDto.archivedAt !== undefined && {
          archivedAt: updateViewDto.archivedAt,
        }),
      },
      userId,
    );
  ...
  async deleteView(
    ...
    // Only creator or project owner can delete a view
    if (existing.createdById !== userId && userRole !== 'owner') {
      throw new ForbiddenException(
        'Only the author or a project owner can delete this saved view',
      );
    }
  ...
  async getFavoriteViews(projectId: string, userId: string): Promise<any[]> {
