...
export class LocalEmbeddingService implements OnModuleInit {
  ...
  onModuleInit(): void {
    this.logger.log(
      'LocalEmbeddingService initialized using decoupled vector embedding engine (384 dimensions).',
    );
   ...
   * Uses built-in deterministic subword vectorizer (fast, zero memory leak, zero external CVE).
   */
  embedText(text: string): Promise<Float32Array> {
    if (!text || !text.trim()) {
      return Promise.resolve(new Float32Array(EMBEDDING_DIMENSIONS));
    }
    return Promise.resolve(this.generateFallbackEmbedding(text.trim()));
  }
