...
export class SemanticSearchService {
  ...
  private async getScopeWhere(userId: string, projectId?: string) {
    if (projectId && projectId !== 'user') {
      const member = await this.repo.checkProjectMember(projectId, userId);
      if (!member) {
  ...
  async searchSemantic(
    ...
    const items = await this.repo.findItemsForSemanticSearch(scopeWhere);
  ...
  async findRelatedItems(
    ...
    const targetItem = await this.repo.findItemWithContributors(
      itemId,
      scopeWhere,
    );
    ...
    const candidates = await this.repo.findCandidateItemsForSemantic(
      itemId,
      scopeWhere,
    );
  ...
  async indexLibrary(
    ...
    const items = await this.repo.findItemsForIndexLibrary(scopeWhere);
