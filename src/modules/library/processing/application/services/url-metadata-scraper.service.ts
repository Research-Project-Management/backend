...
import { MetadataRoutingPolicy } from '../../domain/policies/metadata.policy';
import {
    ...
    if (isPdfContent) {
      try {
        ...
        if (isPdfMagic) {
          ...
          // Extract metadata via ContentFacade
          let pdfExtracted: any = {};
          if (this.contentFacade) {
            try {
              const doc = await this.contentFacade.extractDocumentFromBuffer(
                buffer,
                { headerOnly: true },
              );
              ...
              this.logger.warn(
                `ContentFacade pdf extraction failed: ${pdfErr?.message}`,
              );
            }
          }
