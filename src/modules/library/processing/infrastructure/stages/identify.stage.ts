          ...
          try {
            ...
            if (fileRecord?.buffer) {
              ...
              const extractedDocument =
                await this.contentFacade.extractDocumentFromBuffer(fileRecord.buffer, {
                  headerOnly: true,
                  skipGrobid: true,
                  maxPages: 2,
                });
              ...
              if (
                ...
                extractedMetadata =
                  this.contentFacade.extractMetadataFromBuffer(fileRecord.buffer) || {};
              }
              ...
              if (!detectedDoi && !detectedArxiv && !hasFastIdentifier) {
                try {
                  const grobidDoc = await this.contentFacade.extractDocumentFromBuffer(
                    fileRecord.buffer,
                    {
                      headerOnly: true,
                      skipGrobid: false,
