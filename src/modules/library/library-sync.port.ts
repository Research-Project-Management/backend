...
/**
 * Public Seam & Port for External Integrations syncing with the Library Domain.
 *
 * Encapsulates all transactional boundaries, change logs, tombstones,
 * and outbox event emissions strictly inside the Library domain.
 */
...
export interface DeleteSyncEntityCommand {
  ...
  publishOutboxPayload?: Prisma.JsonValue | Record<string, unknown>;
}

export interface PublishIntegrationEventCommand<T = Prisma.JsonValue | Record<string, unknown>> {
  workspaceId: string;
  aggregateId: string;
  eventType: string;
  dedupeKey?: string;
...
export type ExternalSyncOperation =
  | ({ op: 'upsertCollection'; command: UpsertSyncCollectionCommand } & BaseExternalSyncOperation)
  | ({ op: 'upsertCatalogItem'; command: UpsertSyncCatalogItemCommand } & BaseExternalSyncOperation)
  | ({ op: 'upsertAttachment'; command: UpsertSyncAttachmentCommand } & BaseExternalSyncOperation)
  | ({ op: 'upsertNote'; command: UpsertSyncNoteCommand } & BaseExternalSyncOperation)
  | ({ op: 'upsertAnnotation'; command: UpsertSyncAnnotationCommand } & BaseExternalSyncOperation)
  | ({ op: 'deleteEntity'; command: DeleteSyncEntityCommand } & BaseExternalSyncOperation);
...
export interface IntegrationOutboxEvent<T = Prisma.JsonValue | Record<string, unknown>> {
  id: string;
  workspaceId: string;
  aggregateId: string;
  eventType: string;
