...
export interface ILibraryFeatureFlags {
  ...
  /**
   * Enable Zotero two-way sync push and conflict resolution.
   * Default: false
   */
...
export class LibraryFeatureFlagsService {
  ...
  getFlags(workspaceId?: string): ILibraryFeatureFlags {
    const globalDefaults: ILibraryFeatureFlags = {
      ...
      zoteroTwoWaySync: this.parseBooleanEnv(
        'LIBRARY_ZOTERO_TWO_WAY_SYNC',
        false,
      ),
    };
  ...
  isZoteroTwoWaySyncEnabled(workspaceId?: string): boolean {
    return this.getFlags(workspaceId).zoteroTwoWaySync;
  }

  isFeatureEnabled(featureName: string, workspaceId?: string): boolean {
    const flags = this.getFlags(workspaceId) as unknown as Record<
      string,
      boolean
    >;
    if (featureName === 'zotero_two_way_sync') {
      return this.isZoteroTwoWaySyncEnabled(workspaceId);
    }
