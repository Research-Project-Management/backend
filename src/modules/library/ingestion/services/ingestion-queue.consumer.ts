...
export class IngestionQueueConsumer extends WorkerHost implements OnApplicationBootstrap {
  ...
  onApplicationBootstrap() {
    try {
      ...
      this.logger.log(`IngestionQueueConsumer registered with BullMQ on queue: ${LIBRARY_INGESTION_QUEUE} (concurrency: 4)`);
    } catch {
      // ignore
    }
  }
  ...
  async process(job: Job<QueuedIngestionJob>): Promise<void> {
    ...
    try {
      await this.repo
        ...
        .catch((statusErr: any) => {
          this.logger.warn(`Failed to set DETECTED status for run ${runId}: ${statusErr?.message}`);
        });

      await this.pipeline.executePipeline(runId, projectId, envelope);

      const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
      this.logger.log(`[BULLMQ_DONE] Ingestion run ${runId} completed successfully in ${elapsed}s`);
    } catch (err: any) {
      const elapsed = ((Date.now() - startTime) / 1000).toFixed(1);
      ...
      if (isLastAttempt) {
        await this.repo
          .updateRunStatus(projectId, runId, IngestionStatus.FAILED_FINAL, {
            lastError: err?.message || 'Ingestion pipeline execution failed permanently',
          })
          .catch((updateErr: any) => {
            this.logger.error(`Failed to update run status for failed run ${runId}: ${updateErr?.message || updateErr}`);
          });
      }
