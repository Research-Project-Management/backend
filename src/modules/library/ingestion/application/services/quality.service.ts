...
export class QualityService {
  ...
  calculateItemCompleteness(item: Record<string, any>): CompletenessAnalysis {
    ...
    const fields = this.bibliographyFacade
      ? this.bibliographyFacade.getOrderedFields(itemType)
      : [];
  ...
  async getQualityAudit(userId: string, projectId?: string) {
    const items = this.bibliographyFacade
      ? await this.bibliographyFacade.findQualityAuditItems(
          userId,
          2000,
          projectId,
        )
