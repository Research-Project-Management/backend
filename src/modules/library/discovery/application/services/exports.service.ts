...
import {
  ITEM_READ_PORT,
  IItemReadPort,
} from '../../../catalog/domain/ports/items.ports';
import {
  CONTENT_FACADE,
  IContentFacade,
} from '../../../content/content.facade';

import { PdfBakerService } from './pdf-baker.service';
import { ExportResult, BurnableAnnotation } from '../../domain/types/exports.types';
...
export class ExportsService {
  ...
  async exportBundle(userId: string, collectionId: string) {
    ...
    for (const it of items) {
      ...
      if (this.contentFacade) {
        const attRes = await this.contentFacade.getItemAttachments(userId, it.id);
  ...
  async exportAnnotatedItemPdf(
    ...
    let attachments: any[] = [];
    if (this.contentFacade) {
      const attRes = await this.contentFacade.getItemAttachments(userId, itemId);
      attachments = Array.isArray(attRes) ? attRes : attRes?.attachments || [];
