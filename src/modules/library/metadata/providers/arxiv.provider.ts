...
export class ArxivProvider {
  ...
  async fetchById(arxivId: string): Promise<UnifiedAcademicMetadata | null> {
    ...
    const responseResult = await tryCatch(
      fetch(url, {
        headers: {
          'User-Agent':
            'ResearchManagement/1.0 (mailto:admin@researchmanagement.local; academic-research-bot)',
        },
  ...
  parseXmlPayload(xml: string, cleanId: string): UnifiedAcademicMetadata | null {
    ...
    const fullPublishedMatch = xml.match(/<published>([\s\S]*?)<\/published>/i);
    const publicationDate =
      fullPublishedMatch && fullPublishedMatch[1]
        ? fullPublishedMatch[1].split('T')[0]
        : undefined;
    ...
    const journalRefMatch = xml.match(
      /<arxiv:journal_ref[^>]*>([\s\S]*?)<\/arxiv:journal_ref>/i,
    );
    const journalRef =
      journalRefMatch && journalRefMatch[1]
        ? journalRefMatch[1].replace(/\s+/g, ' ').trim()
        : undefined;

    const primaryCatMatch = xml.match(
      /<arxiv:primary_category[^>]*term="([^"]+)"/i,
    );
    const primaryCategory =
      primaryCatMatch && primaryCatMatch[1] ? primaryCatMatch[1] : undefined;
