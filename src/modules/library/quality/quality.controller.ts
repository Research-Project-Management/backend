...
import { MergePapersDto } from './dto/quality.dto';
...
export class QualityController {
  ...
  @ApiOperation({
    summary: 'Get 2-tier duplicate paper groups in workspace (DOI and Fuzzy Title/Year/Author matching)',
  })
  ...
  @ApiOperation({
    summary: 'Execute safe merge: consolidates notes/labels, transfers attachments to master, and soft-deletes sources',
  })
  async mergePapers(
    @Param('workspaceId') workspaceId: string,
    @CurrentUser('id') userId: string,
    @Body() dto: MergePapersDto,
  ) {
  ...
  @ApiOperation({
    summary: 'Scan library metadata integrity and return diagnostic health report',
  })
