...
import { extractFamilyName } from '../utils/name-parser.util';
...
export class CslFormatter {
  ...
  formatEntry(
    ...
    const title = paper.title ? paper.title.trim().replace(/\.$/, '') : 'Untitled';
    ...
    switch (style.toLowerCase() as CitationStyle) {
      case 'ieee':
        return this.formatIeee(paper, index, authors, year, title, journal, volume, issue, pages, doiUrl);
      case 'nature':
        return this.formatNature(paper, authors, year, title, journal, volume, pages, doiUrl);
      case 'harvard':
        return this.formatHarvard(paper, authors, year, title, journal, volume, issue, pages, doiUrl);
      case 'chicago':
        return this.formatChicago(paper, authors, year, title, journal, volume, issue, pages, doiUrl);
      case 'mla':
        return this.formatMla(paper, authors, year, title, journal, volume, issue, pages, doiUrl);
      case 'vancouver':
        return this.formatVancouver(paper, index, authors, year, title, journal, volume, issue, pages, doiUrl);
      case 'apa':
      default:
        return this.formatApa(paper, authors, year, title, journal, volume, issue, pages, doiUrl);
    }
  }
  ...
  private formatAuthorsMla(authors: string[]): string {
    ...
    if (authors.length === 2) {
      ...
      const secStr = second.given ? `${second.given} ${second.family}` : second.family;
