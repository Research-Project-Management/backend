...
@Injectable()
export class SemanticScholarFetcher {
  private readonly logger = new Logger(SemanticScholarFetcher.name);
  ...
  async fetchById(paperId: string): Promise<UnifiedAcademicMetadata | null> {
    ...
    if (!responseResult.ok) {
      this.logger.warn(`S2 fetch error for ${paperId}: ${getErrorMessage(responseResult.error)}`);
      return null;
    }
    ...
    const jsonResult = await tryCatch(response.json() as Promise<any>);
  ...
  async searchByTitle(title: string): Promise<UnifiedAcademicMetadata | null> {
    ...
    if (!jsonResult.ok || !jsonResult.value.data || jsonResult.value.data.length === 0) {
      return null;
    }
  ...
  private transformPayload(data: any): UnifiedAcademicMetadata {
    ...
    if (Array.isArray(data.publicationTypes)) {
      if (data.publicationTypes.includes('Conference') || data.publicationTypes.includes('ConferencePaper')) {
