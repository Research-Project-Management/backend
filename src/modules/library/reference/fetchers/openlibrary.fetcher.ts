...
@Injectable()
export class OpenlibraryFetcher {
  private readonly logger = new Logger(OpenlibraryFetcher.name);
  ...
  async fetchByIsbn(isbn: string): Promise<UnifiedAcademicMetadata | null> {
    ...
    const publisher = Array.isArray(item.publishers) && item.publishers[0]?.name
      ? item.publishers[0].name
      : undefined;

    const pages = item.number_of_pages ? String(item.number_of_pages) : undefined;
