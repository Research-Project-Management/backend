...
@Injectable()
export class UnpaywallFetcher {
  private readonly logger = new Logger(UnpaywallFetcher.name);
  ...
  async resolveOaPdf(doi: string): Promise<UnpaywallResult | null> {
    ...
    const responseResult = await tryCatch(
      fetch(url, {
        headers: {
          'User-Agent': 'ResearchManagementPlatform/1.0 (academic-research-bot)',
    ...
    const pdfUrl = bestOaLocation?.url_for_pdf || bestOaLocation?.url || undefined;
