...
import {
  toLibraryItemListResult,
  toLibraryItemResponse,
} from './items/item.mapper';
import { LibraryItemListQuery, LibraryItemRecord } from './items/item.types';
import {
...
export class CatalogService {
  ...
  private toIngestedItemResponse(res: {
    item?: LibraryItemRecord;
    paper?: LibraryItemRecord;
  }) {
    const item = res.item ?? res.paper;
    if (!item) {
      throw new NotFoundException('Ingestion did not return a library item');
    }
  ...
  async getPapers(workspaceId: string, query?: LibraryItemListQuery) {
    ...
    const [papers, total] = await Promise.all([
      ...
      this.catalogRepo.countPapers(where),
    ]);
  ...
  async uploadPaper(
    ...
    return this.toIngestedItemResponse(res);
  }
  ...
  async ingestPaper(
    ...
    const sourceType = dto.doi?.trim()
      ? IngestionSourceType.IDENTIFIER
  ...
  async exportBibtex(itemId: string) {
    const paper = await this.catalogRepo.findItemById(itemId);
    if (!paper) throw new NotFoundException('Catalog item not found');
    return this.bibtexFormatter.formatEntry(paper);
  }

  async exportBibtexInWorkspace(workspaceId: string, itemId: string) {
    const item = await this.resolveItemInWorkspace(workspaceId, itemId);
    return this.bibtexFormatter.formatEntry(item);
  }
