...
describe('Update Module', () => {
  ...
  describe('getUpdates', () => {
    it('should throw NotFoundException if WorkItem not found', async () => {
      ...
      await expect(service.getUpdates(taskId)).rejects.toThrow(
        NotFoundException,
      );
    });
      ...
      mockPrisma.workItem.findUnique.mockResolvedValue({
        ...
        updates: [
          ...
          },
          {
            ...
            status: WorkItemUpdateStatus.AT_RISK,
          },
        ],
      });
  ...
  describe('addUpdate', () => {
    it('should throw NotFoundException if WorkItem not found', async () => {
      mockPrisma.workItem.findUnique.mockResolvedValue(null);
      await expect(
        service.addUpdate(taskId, authorId, {
          status: WorkItemUpdateStatus.ON_TRACK,
        }),
  ...
  describe('deleteUpdate', () => {
    it('should throw NotFoundException if update not found', async () => {
      ...
      await expect(
        service.deleteUpdate(taskId, 'u-missing', authorId),
      ).rejects.toThrow(NotFoundException);
    });
