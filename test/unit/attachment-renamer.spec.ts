...
} from '@/modules/library/reader/application/utils/renamer.util';
...
describe('Attachment Renamer Utility Spec (Zotero 7 Official Standard)', () => {
  it('should format filename with official Zotero 7 default template {{ firstCreator suffix=" - " }}{{ year suffix=" - " }}{{ title truncate="100" }}', () => {
    ...
    const result = formatAttachmentFilename(
      DEFAULT_RENAME_PATTERN,
      item,
      'original.pdf',
    );
  ...
  it('should format filename with authors et al. pattern when specified', () => {
    const pattern =
      '{{ authors suffix=" - " }}{{ year suffix=" - " }}{{ title truncate="100" }}';
    const item = {
    ...
    expect(result).toBe(
      'He and Zhang - 2016 - Deep Residual Learning for Image Recognition.pdf',
    );
  });
  ...
  it('should format filename with 3+ authors using "et al."', () => {
    ...
    const item = {
      ...
      contributors: [
        {
          fullName: 'Ian Goodfellow',
          lastName: 'Goodfellow',
          creatorType: 'author',
          orderIndex: 0,
        },
        {
          fullName: 'Jean Pouget-Abadie',
          lastName: 'Pouget-Abadie',
          creatorType: 'author',
          orderIndex: 1,
        },
        {
          fullName: 'Mehdi Mirza',
          lastName: 'Mirza',
          creatorType: 'author',
          orderIndex: 2,
        },
    ...
    expect(result).toBe(
      'Goodfellow et al. - 2014 - Generative Adversarial Nets.pdf',
    );
  });
  ...
  it('should sanitize illegal characters and colon in title', () => {
    ...
    expect(result).toBe(
      'Devlin - 2019 - BERT Pre-training of Deep Bidirectional Transformers for Language Understanding.pdf',
    );
  });
