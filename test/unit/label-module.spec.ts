...
describe('Work Item Label Module', () => {
  ...
  describe('User-Scoped Personal Labels', () => {
    it('should route createLabel with projectId to createProjectLabel', async () => {
      (mockLabelRepo.findByNameInProject as jest.Mock).mockResolvedValue(null);
      (mockLabelRepo.create as jest.Mock).mockImplementation((data) =>
        Promise.resolve({ id: 'created-1', ...data }),
      );

      const result = await service.createLabel(mockUserId, {
        name: 'Project Tag',
        projectId: mockProjectId,
      });
    ...
    it('should return labels for user when no projectId is provided', async () => {
      (mockLabelRepo.findUserLabels as jest.Mock).mockResolvedValue([
        { id: 'user-label-1', name: 'Personal Label' },
      ]);

      const result = await service.getLabels(mockUserId);

      expect(result.labels).toHaveLength(1);
      expect(mockLabelRepo.findUserLabels).toHaveBeenCalledWith(
        mockUserId,
        undefined,
        undefined,
      );
