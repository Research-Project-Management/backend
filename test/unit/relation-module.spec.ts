...
describe('Relation Module (Work Item Dependencies & Bidirectional Linking)', () => {
  ...
  const baseTaskB: any = {
    ...
    identifier: 'FLUX-2',
    sequenceNumber: 2,
    title: 'WorkItem B (User dashboard)',
  ...
  describe('Add Relation', () => {
    it('should add bidirectional blocks / blocked_by relation between two tasks', async () => {
      ...
      // Verify WorkItem B gets "blocked_by"
      expect(mockPrisma.workItem.update).toHaveBeenCalledWith(
        expect.objectContaining({
          where: { id: taskBId },
          data: {
    ...
    it('should reject linking a WorkItem to itself', async () => {
    ...
    it('should throw NotFoundException if source WorkItem does not exist', async () => {
    ...
    it('should throw NotFoundException if target WorkItem does not exist', async () => {
      mockRelationRepo.findTask!.mockImplementation((id: string) => {
        if (id === taskAId) return Promise.resolve({ ...baseTaskA });
        return Promise.resolve(null);
      });
  ...
  describe('Remove Relation', () => {
    it('should remove bidirectional relation atomically from both tasks', async () => {
      ...
      mockRelationRepo.findTask!.mockImplementation((id: string) => {
        if (id === taskAId) return Promise.resolve(existingTaskA);
        if (id === taskBId) return Promise.resolve(existingTaskB);
        return Promise.resolve(null);
      });
      ...
      // Verify WorkItem A has relations filtered
      expect(mockPrisma.workItem.update).toHaveBeenCalledWith({
        where: { id: taskAId },
        data: { relations: [] },
      });

      // Verify WorkItem B has relations filtered
      expect(mockPrisma.workItem.update).toHaveBeenCalledWith({
        where: { id: taskBId },
        data: { relations: [] },
      });
    ...
    it('should throw NotFoundException if source WorkItem not found on removal', async () => {
      mockRelationRepo.findTask!.mockResolvedValue(null);
  ...
  describe('Get WorkItem Relations with Target Hydration', () => {
    it('should return empty array if WorkItem has no relations', async () => {
    ...
    it('should enrich relations with target WorkItem metadata', async () => {
      ...
      mockRelationRepo.findTask!.mockResolvedValue(taskWithRelations);
