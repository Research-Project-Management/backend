...
describe('Comment Module (WorkItem Discussions & Threading)', () => {
  ...
  describe('Comment Retrieval & Creation', () => {
    it('should retrieve all comments for a task', async () => {
      ...
      mockCommentRepo.findTaskComments.mockResolvedValue(mockComments);
    ...
    it('should create a new comment on a work item', async () => {
      ...
      mockCommentRepo.createTaskComment.mockResolvedValue(createdComment);
  ...
  describe('Comment Authorization & Updates', () => {
    const existingComment = {
      ...
      task: {
        ...
        project: { id: mockProjectId, createdById: 'some-creator' },
      },
    };
    ...
    it('should allow author to update their own comment', async () => {
      ...
      mockCommentRepo.updateTaskComment.mockResolvedValue({
        ...existingComment,
    ...
    it('should allow project creator to update a comment even if not author', async () => {
      mockCommentRepo.findTaskCommentWithProject.mockResolvedValue({
        ...existingComment,
        task: {
          projectId: mockProjectId,
          project: { id: mockProjectId, createdById: mockOtherUserId },
        },
      } as any);
      mockCommentRepo.updateTaskComment.mockResolvedValue({
        ...existingComment,
        content: 'Moderated by project creator',
        isEdited: true,
      });
      ...
      expect(result.comment.content).toBe('Moderated by project creator');
    });

    it('should allow project owner to update a comment even if not author', async () => {
      mockCommentRepo.findTaskCommentWithProject.mockResolvedValue(
        existingComment as any,
      );
      mockCommentRepo.findProjectMemberRole.mockResolvedValue('owner');
      mockCommentRepo.updateTaskComment.mockResolvedValue({
        ...existingComment,
        content: 'Moderated by project owner',
        isEdited: true,
      });
      ...
      expect(result.comment.content).toBe('Moderated by project owner');
    });

    it('should throw ForbiddenException if user is not author and lacks owner role', async () => {
      mockCommentRepo.findTaskCommentWithProject.mockResolvedValue(
        existingComment as any,
      );
  ...
  describe('Threading & Replies', () => {
    it('should append a threaded reply to a comment', async () => {
      mockCommentRepo.findTaskCommentById.mockResolvedValue({
        id: mockCommentId,
        replies: [],
      });
      mockCommentRepo.updateTaskComment.mockImplementation((_id, data) => ({
        id: mockCommentId,
    ...
    it('should throw NotFoundException if replying to non-existent comment', async () => {
      mockCommentRepo.findTaskCommentById.mockResolvedValue(null);
