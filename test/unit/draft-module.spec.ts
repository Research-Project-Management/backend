...
describe('Draft Module', () => {
  ...
  describe('getDraft', () => {
    it('should throw NotFoundException if draft not found', async () => {
      ...
      await expect(service.getDraft(draftId, authorId)).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe('publishDraft', () => {
    it('should throw NotFoundException if draft does not exist', async () => {
      mockRepo.findById.mockResolvedValue(null);
      await expect(service.publishDraft(draftId, {}, authorId)).rejects.toThrow(
        NotFoundException,
      );
    });
    ...
    it('should convert draft to work item, delete draft, and emit event', async () => {
      ...
      mockWorkItemService.createTask.mockResolvedValue({
        WorkItem: { id: 'task-real-1', identifier: 'FLUX-101' },
      });
  ...
  describe('duplicateDraft', () => {
    it('should throw NotFoundException if draft not found', async () => {
      mockRepo.findById.mockResolvedValue(null);
      await expect(service.duplicateDraft(draftId, authorId)).rejects.toThrow(
        NotFoundException,
      );
    });
  ...
  describe('deleteDraft', () => {
    it('should throw NotFoundException if draft not found', async () => {
      mockRepo.findById.mockResolvedValue(null);
      await expect(service.deleteDraft(draftId, authorId)).rejects.toThrow(
        NotFoundException,
      );
    });
