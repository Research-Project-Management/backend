...
describe('ItemTransformer (Matt Pocock Pattern)', () => {
  ...
  describe('previewConversion', () => {
    ...
    it('tracks dropped fields and retains unmapped in extra fields when enabled', () => {
      ...
      expect(
        (result.projectedItem.extraFields as any)[
          '__unmapped_journalArticle_issue'
        ],
      ).toBe('12');
    });
    ...
    it('handles creator role changes with primary and secondary fallback', () => {
      ...
      expect((result.projectedItem.creators as any)[0].creatorType).toBe(
        'author',
      );
      ...
      expect((result.projectedItem.creators as any)[1].creatorType).toBe(
        'contributor',
      );
    });
  });
