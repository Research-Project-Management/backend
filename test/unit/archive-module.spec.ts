...
describe('Archive Module', () => {
  ...
  describe('archiveWorkItem', () => {
    it('should throw NotFoundException if WorkItem not found', async () => {
      mockRepo.findTaskWithProject.mockResolvedValue(null);
      await expect(service.archiveWorkItem(taskId, userId)).rejects.toThrow(
        NotFoundException,
      );
    });

    it('should throw BadRequestException if WorkItem is already archived', async () => {
    ...
    it('should archive WorkItem and emit events', async () => {
      const mockTask = {
        id: taskId,
        identifier: 'FLUX-1',
        projectId,
      ...
      mockRepo.archiveTask.mockResolvedValue({
        ...mockTask,
        archivedAt: new Date(),
      });
  ...
  describe('restoreWorkItem', () => {
    it('should throw NotFoundException if WorkItem not found', async () => {
      mockRepo.findTaskWithProject.mockResolvedValue(null);
      await expect(service.restoreWorkItem(taskId, userId)).rejects.toThrow(
        NotFoundException,
      );
    });

    it('should throw BadRequestException if WorkItem is not archived', async () => {
      mockRepo.findTaskWithProject.mockResolvedValue({
        id: taskId,
        archivedAt: null,
        identifier: 'FLUX-1',
    ...
    it('should restore WorkItem and emit events', async () => {
      const mockTask = {
        id: taskId,
        identifier: 'FLUX-1',
        projectId,
  ...
  describe('bulkArchiveWorkItems', () => {
    ...
    it('should bulk archive tasks and return count', async () => {
      ...
      const result = await service.bulkArchiveWorkItems(
        { taskIds: ['t1'] },
        userId,
      );
      expect(result.success).toBe(true);
  ...
  describe('bulkRestoreWorkItems', () => {
    ...
    it('should bulk restore tasks and return count', async () => {
      ...
      const result = await service.bulkRestoreWorkItems(
        { taskIds: ['t1'] },
        userId,
      );
      expect(result.success).toBe(true);
  ...
  describe('getArchivedWorkItems', () => {
    it('should query archived tasks for project', async () => {
      ...
      const result = await service.getArchivedWorkItems(projectId, {
        page: 1,
        limit: 10,
      });
      expect(result.total).toBe(0);
      expect(mockRepo.findArchivedTasks).toHaveBeenCalledWith(projectId, {
        page: 1,
        limit: 10,
      });
    });
