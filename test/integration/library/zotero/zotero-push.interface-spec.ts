...
import { ZoteroPushWorker } from '../../../../src/modules/library/integrations/zotero/zotero-push.worker';
import { ZoteroConnector } from '../../../../src/modules/library/integrations/zotero/zotero.connector';
import { ZoteroConnectionService } from '../../../../src/modules/library/integrations/zotero/zotero-connection.service';
import { ZoteroMapper } from '../../../../src/modules/library/integrations/zotero/zotero.mapper';
import { ZoteroConflictService } from '../../../../src/modules/library/integrations/zotero/zotero-conflict.service';
...
describe('Zotero Two-Way Push & Three-Way Conflict Resolution (Integration T079/T080/T081)', () => {
  ...
  beforeAll(async () => {
    ...
    syncPolicy = new ZoteroSyncPolicy(harness.featureFlags);
  ...
  it('1. Clean Two-Way Push: pushes local catalog item and records remote binding + outbox', async () => {
    ...
    // Execute Push
    const result = await pushWorker.pushCatalogItem(
      tenant.workspaceId,
      binding.id,
      item.id,
    );
  ...
  it('3. Three-Way Conflict Detection & Resolution Workflow', async () => {
    ...
    const conflictResult = await pushWorker.pushCatalogItem(
      tenant.workspaceId,
      binding.id,
      item.id,
    );
  ...
  it('4. Kill-Switch & Policy: halts outgoing push when disabled without failing local ops', async () => {
    ...
    syncPolicy.setGlobalPushKillSwitch(true);
    ...
    // Reset kill-switch
    syncPolicy.setGlobalPushKillSwitch(false);
  });
